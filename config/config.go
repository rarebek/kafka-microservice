...
func Load() Config {
	...
	c.RedisAddr = cast.ToString(getOrReturnDefault("REDIS_ADDR", "localhost"))
	...
	c.KafkaAddress = cast.ToString(getOrReturnDefault("KAFKA_ADDRESS", "localhost:9092"))
	c.KafkaTopic = cast.ToString(getOrReturnDefault("KAFKA_TOPIC", "rare"))
